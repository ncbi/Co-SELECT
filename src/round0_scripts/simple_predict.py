...
def getCycle0ProbFile(ctx, shape, motif, dist, lflank, rflank, shape_levels_str):
  fname = "/panfs/pan1/aptax/simple_predict/prob.%s.%s.%s.%s" % (ctx, motif, shape, shape_levels_str)
  fname += ".d%s" % (dist)
  if ctx == 'fg':
    fname += ".l%d.r%d" % (lflank, rflank)
  fname += ".txt"
