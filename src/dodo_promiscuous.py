    ...
    for shape_type in shapes:
      for levels_type in discrete_levels_type:
        for lflank, rflank in flank_configs:
          enriched = '%s/%s/%s/denriched_%s.%s.l%d.r%d.csv' % (top_results_dir, levels_type, en_th, 'same', shape_type, lflank, rflank)
          output = '%s/%s/%s/promiscuous_%s.%s.l%d.r%d.csv' % (top_results_dir, levels_type, en_th, 'same', shape_type, lflank, rflank)
          yield {
            'name'      : output,
...
def task_combine_promiscuous():
  ...
  for levels_type in discrete_levels_type:
    for lflank, rflank in flank_configs:
      infiles = ['%s/%s/%s/promiscuous_%s.%s.l%d.r%d.csv' % (top_results_dir, levels_type, en_th, 'same', shape_type, lflank, rflank) for en_th in en_thresholds for shape_type in shapes]
      infos = [(en_th, shape_type) for en_th in en_thresholds for shape_type in shapes]
      output = '%s/promiscuous_%s_l%d.r%d.csv' % (top_results_dir, levels_type, lflank, rflank)
      yield {
        'name'      : output,
