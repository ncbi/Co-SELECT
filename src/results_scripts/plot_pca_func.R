...
getPCAplot <- function(pca, show_labels=TRUE) {
  ...
  ## Override the guide aesthetics to make them look like fill colors  
    guides(shape = guide_legend(order=2,
               override.aes = list(colour = mycolors$color[mycolors$family=='ETS'], fill = NA, shape = 15, size = 7),
               title="ETS", label.theme=element_text(family='mono')),
           linetype = guide_legend(order=1,
               override.aes = list(colour = mycolors$color[mycolors$family=='bHLH'], shape = 15, size = 7),
               title = "bHLH", label.theme=element_text(family='mono')),
           fill = guide_legend(order=3,
               override.aes = list(colour = mycolors$color[mycolors$family=='homeodomain'], shape = 15,size = 7),
               title = "homeodomain", label.theme=element_text(family='mono', lineheight=0.75))) +
