...
promiscuous_file = paste0(top_res_dir, "/promiscuous_same.", shape,'.', cycle, ".l1.r1.csv") 
...
seq2shape <- fread(paste0('/panfs/pan1/aptax/seqmer2shapemer/map_seq2shape_', shape, '.csv'), stringsAsFactors=FALSE)
print(seq2shape)

num_sequences <- merge(all_promiscuous, seq2shape, by.x='kmer', by.y='shapemer')[,.(numseq=.N), by=.(kmer)]
print(num_sequences)

data_file <- paste(top_res_dir, paste0("bg_summary_", shape, ".csv"), sep='/')
out_file <- paste(top_res_dir, paste0("promiscuous_significance_", shape, ".csv"), sep='/')
