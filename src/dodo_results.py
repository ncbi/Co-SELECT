          ...
          for levels_type in discrete_levels_type:
            ...
            for cycle in task.cycles:
              for motif, dist in izip(task.motifs, task.distances):
                  ...
                  prob_files = [getCycle0ProbFile(probability_dir, ctx, shape_type, motif, dist, lflank, rflank, shape_levels_str) for ctx in contexts]
                  yield {
                    'name'      : output_file,
                    'actions'   : ["results_scripts/get_summary.R %s %s %d %s %s %d %d %d %s %s %s %s %s %s" % (task.tf, task.primer, cycle, shape_type, motif, dist, lflank, rflank, en_th, resdir, shape_levels_str, task.family, top_data_dir, probability_dir)],
        ...
        for lflank, rflank in flank_configs:
          for shape_type in shapes:
            for levels_type in discrete_levels_type:
              ...
              input_files = [top_data_dir +'/'+ info1.getContextedShapemerCountFile(int(cycle), motif1, int(dist1), lflank, rflank, 'fg', shape_type, shape_levels_str)]
              input_files += [top_data_dir +'/'+ info2.getContextedShapemerCountFile(int(cycle), motif2, int(dist2), lflank, rflank, 'bg', shape_type, shape_levels_str)]
              file_fisher = '.'.join(['fisher', tf1, primer1, motif1, str(cycle), tf2, primer2, motif2, str(cycle), shape_type, str(lflank), str(rflank), 'csv'])
              output_file = '/'.join([resdir, file_fisher])
              prob_files = [getCycle0ProbFile(probability_dir, 'fg', shape_type, motif1, dist1, lflank, rflank, shape_levels_str)]
              prob_files += [getCycle0ProbFile(probability_dir, 'bg', shape_type, motif2, dist2, lflank, rflank, shape_levels_str)]
              yield {
                'name'      : output_file,
                'actions'   : ["results_scripts/get_cross_summary.R %s %s %s %s %d %d %s %s %s %s %d %d %s %d %d %s %s %s %s %s" % (tf1, primer1, family1, motif1, dist1, cycle, tf2, primer2, family2, motif2, dist2, cycle, shape_type, lflank, rflank, en_th, resdir, shape_levels_str, top_data_dir, probability_dir)],
                'file_dep'  : input_files + prob_files,
                'targets'   : [output_file],
                'clean'     : True,
              }
  ...
  for en_th in en_thresholds:
    for cycle in cycles:
      for lflank, rflank in flank_configs:
        for shape_type in shapes:
          for levels_type in discrete_levels_type:
            inputs = ['/'.join([top_results_dir, levels_type, en_th, task.family, task.tf, task.primer, '.'.join(['enriched', task.tf, task.primer, shape_type, str(cycle), motif, str(lflank), str(rflank), 'csv'])]) for task in task_infos for motif, dist in izip(task.motifs, task.distances)]
            ...
            target = '%s/%s/%s/denriched_%s.%s.%d.l%d.r%d.csv' % (top_results_dir, levels_type, en_th, 'same', shape_type, cycle, lflank, rflank)
            yield {
              'name'      : target,
  ...
  for en_th in en_thresholds:
    for cycle in cycles:
      for lflank, rflank in flank_configs:
        for shape_type in shapes:
          for levels_type in discrete_levels_type:
            inputs = ['/'.join([top_results_dir, levels_type, en_th, task.family, task.tf, task.primer, '.'.join(['fisher', task.tf, task.primer, shape_type, str(cycle), motif, str(lflank), str(rflank), 'csv'])]) for task in task_infos for motif, dist in izip(task.motifs, task.distances)]
            infos = [(task.tf, task.primer, task.family, task.tf, task.primer, task.family) for task in task_infos for motif, dist in izip(task.motifs, task.distances)]
            target = '%s/%s/%s/dfisher_%s.%s.%d.l%d.r%d.csv' % (top_results_dir, levels_type, en_th, 'same', shape_type, cycle, lflank, rflank)
            yield {
              'name'      : target,
              #'actions'   : ["results_scripts/fisher.R %s %s %s %d %d %s" % (shape_type, en_th, 'same', lflank, rflank, levels_type)],
              'actions'   : [(combine_data_frames, [inputs, infos, target])],
  ...
  for en_th in en_thresholds:
    for cycle in cycles:
      for lflank, rflank in flank_configs:
        for shape_type in shapes:
          for levels_type in discrete_levels_type:
            inputs = ['/'.join([top_results_dir, levels_type, en_th, resdir, '.'.join(['fisher', tf1, primer1, motif1, str(cycle), tf2, primer2, motif2, str(cycle), shape_type, str(lflank), str(rflank), 'csv'])]) for resdir, tf1, primer1, motif1, family1, dist1, tf2, primer2, motif2, family2, dist2 in combinations]
            infos = [(tf1, primer1, family1, tf2, primer2, family2) for resdir, tf1, primer1, motif1, family1, dist1, tf2, primer2, motif2, family2, dist2 in combinations]
            target = '%s/%s/%s/dfisher_%s.%s.%d.l%d.r%d.csv' % (top_results_dir, levels_type, en_th, 'cross', shape_type, cycle, lflank, rflank)
            yield {
              'name'      : target,
          ...
          for levels_type in discrete_levels_type:
            for family in ['bHLH', 'ETS', 'homeodomain']:
              for exp_type in ['same', 'cross']:
                infile = '%s/%s/%s/dfisher_%s.%s.%d.l%d.r%d.csv' % (top_results_dir, levels_type, en_th, exp_type, shape_type, cycle, lflank, rflank)
                outfile = '%s/%s/%s/dqvalue_%s.%s.%s.%d.l%d.r%d.csv' % (top_results_dir, levels_type, en_th, exp_type, family, shape_type, cycle, lflank, rflank)
                yield {
                  'name'      : outfile,
...
def task_combine_all_qvalues():
  for lflank, rflank in flank_configs:
    for cycle in cycles:
      infiles = ['%s/%s/%s/dqvalue_%s.%s.%s.%d.l%d.r%d.csv' % (top_results_dir, levels_type, en_th, exp_type, family, shape_type, cycle, lflank, rflank)  for en_th in en_thresholds     for shape_type in shapes for levels_type in discrete_levels_type for family in ['bHLH', 'ETS', 'homeodomain'] for exp_type in ['same', 'cross']]
